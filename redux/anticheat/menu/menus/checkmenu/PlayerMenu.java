...
public class PlayerMenu extends Menu {
	...
	public PlayerMenu() {
		super("§d§lPlayer §8(§f§l" + Main.getInstance().getCheckManager().getChecks(Category.PLAYER) + " checks§8)", 45);
	
		int slot = 0;
		ArrayList<String> lore = new ArrayList<String>();
		for(PacketCheck c : Main.getInstance().getCheckManager().get(Category.PLAYER)) {
