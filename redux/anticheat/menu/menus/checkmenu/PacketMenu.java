...
public class PacketMenu extends Menu {
	...
	public PacketMenu() {
		super("§d§lPackets §8(§f§l" + Main.getInstance().getCheckManager().getChecks(Category.PACKETS) + " checks§8)",
				45);

		int slot = 0;
		final ArrayList<String> lore = new ArrayList<String>();
		for (final PacketCheck c : Main.getInstance().getCheckManager().get(Category.PACKETS)) {
