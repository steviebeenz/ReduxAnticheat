...
public class MovementMenu extends Menu {
	...
	public MovementMenu() {
		super("§d§lMovement §8(§f§l" + Main.getInstance().getCheckManager().getChecks(Category.MOVEMENT) + " checks§8)", 45);
	
		int slot = 0;
		ArrayList<String> lore = new ArrayList<String>();
		for(PacketCheck c : Main.getInstance().getCheckManager().get(Category.MOVEMENT)) {
