...
public class CombatMenu extends Menu {
	...
	public CombatMenu() {
		super("§d§lCombat §8(§f§l" + Main.getInstance().getCheckManager().getChecks(Category.COMBAT) + " checks§8)",
				45);

		int slot = 0;
		final ArrayList<String> lore = new ArrayList<String>();
		for (final PacketCheck c : Main.getInstance().getCheckManager().get(Category.COMBAT)) {
