...
public class Criticals extends PacketCheck {
	...
	public Criticals() {
		super("Criticals", 5, null, false, true, Category.COMBAT, new PacketType[] { PacketType.Play.Client.USE_ENTITY }, false, 65);
	}
	...
	public void listen(PacketEvent pe) {
		...
		if(pd.getNextLocation().getY() - pd.getLastLocation().getY() % 2 == 0) {
			if(!locUtils.isInLiquid(pd.getNextLocation()) && !locUtils.isCollidedWeb(pd.getLastLocation(), pd.getNextLocation())) {
				flag(pd, "Bruh");
			}
		}
	}
