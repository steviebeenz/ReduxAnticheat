...
public class Speed extends PacketCheck {
	...
	public Speed() {
		...
		this.setDescription("Checks if a player's speed is more than a dynamic limit.");
	}
			...
			if (pd.onGroundTicks > 0) {
				if (Main.getInstance().getLocUtils().isOnSolidGround(pd.getNextLocation())) {
					...
					if(pd.getDeltaXZ() > maxDist) {
						...
						//p.sendMessage("flagged: " + (pd.getDeltaXZ() - maxDist) + ", ground: " + pd.onGroundTicks + ", velocity: " + Math.abs(pd.getVelocity()) * 0.12);
						//p.sendMessage("added: " + ReflectionUtils.getPingModifier(p) * 0.18 + " in ping");
					}
				}
			}
