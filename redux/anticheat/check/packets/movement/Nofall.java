...
public class Nofall extends PacketCheck {
	...
	public Nofall() {
		...
		this.setDescription("Checks if a player is changing their ground packet.");
	}
		...
		if (!ReflectionUtils.getOnGround(p) && Main.getInstance().getLocUtils().isCollided(p.getLocation(), Material.AIR)) {
			...
			if (!Main.getInstance().getLocUtils().isOnSlime(pd.getNextLocation())
				...
				if (pd.onGroundTicks == 0 && pd.offGroundTicks > 20
						&& !Main.getInstance().getLocUtils().isClimbable(p.getLocation().getBlock().getType())
						&& !Main.getInstance().getLocUtils().canClimb(pd.getLastLocation()) && !Main.getInstance().getLocUtils().canClimb(pd.getNextLocation())
						&& !Main.getInstance().getLocUtils().isInLiquid(p.getLocation()) && !Main.getInstance().getLocUtils().isInLiquid(pd.getNextLocation())
